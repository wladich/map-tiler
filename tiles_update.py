...
proj_gmerc_180 = pyproj.Proj('+proj=merc +a=6378137 +b=6378137 +lat_ts=0.0 +lon_0=180.0 +x_0=20037508.342789244 +y_0=0 +k=1.0 +units=m +nadgrids=@null +wktext  +no_defs')
...
def open_map_reference(filename):
    ...
    if filename.endswith('.map'):
        data = ozi_to_maprec.get_maprecord_from_ozi_data(filename)
        maprecord = map_record.Maprecord(filename, data)
    else:
        maprecord = map_record.Maprecord(filename)
    return maprecord
...
#        fn = os.path.join(self.work_dir, self.prev_state_filename)
...
#        tiles_with_overviews.update(tile_stack(*tile))
...
#    print [x[0] for x in job.get_tiles_needing_update()]
#    print 'Tiles to remove: ', job.get_removed_tiles()
